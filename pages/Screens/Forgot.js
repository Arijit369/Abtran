...
try{
  ...
  .then(() => {
    window.alert("Password Reset Email Send On Your Email")
  })
  .catch((error) => {
        ...
        <form className=" p-3 w-full" action="#"  onSubmit={(e)=>handleSubmit(e)}>
          ...
          <div className="rounded-md  -space-y-px p-5  ">
            ...
            <div className="mt-1">
              ...
              <input id="password" name="password" value={name} onChange={(e)=>SetName(e.target.value)} type="text" autoComplete="current-password"  className="appearance-none rounded relative block w-full  border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm py-3 my-3" placeholder="your Name"/>
            </div>
           
          </div>
          <div className="my-2 p-1 flex justify-center">
                   <Link href="/Screens/Login"><a className='text-indigo-700 text-lg font-serif hover:underline hover:text-blue-700'>Login Now</a></Link>
              </div>
