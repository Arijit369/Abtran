...
export default function ({user}) {
  ...
  const loadcoments = async () => {
    const comments = await db.collection('Paidapps').doc(`${id}`).collection('comments').orderBy('time', 'desc').get()
    setAllComments(comments.docs.map(docSnap => docSnap.data()));
    const download = await db.collection('Paidapps').doc(`${id}`).collection('downloads').get()
    setAlldownloads(download.docs.map(docSnap => docSnap.data()))
    const like = await db.collection('Paidapps').doc(`${id}`).collection('Likes').get()
  ...
  const loadapps = async () => {
    try {
      const docRef = db.collection("Paidapps").doc(id);
  ...
  const Approve = async () => {
    ...
    if (!Email || !myComment) {
      ...
      await db.collection('Paidapps').doc(`${id}`).collection('comments').add({
        text: myComment,
        email: Email,
      ...
      const commentQuery = await db.collection('Paidapps').doc(`${id}`).collection('comments').get()
  ...
  const Download = async () => {
    await db.collection('Paidapps').doc(`${id}`).collection('downloads').add({
      text: 1,
      time: serverTimestamp()
  ...
  const Likes = async () => {
    await db.collection('Paidapps').doc(`${id}`).collection('Likes').add({
      text: 1,
      time: serverTimestamp()
    })
...
db.collection('usercart').add({
   ...
   await router.push('/Screens/Cart')
          ...
          <Slide bottom>
            <div className="  flex lg:flex-row flex-col  lg:p-10 md:p-8 sm:p-5 p-4  justify-between gap-5">
              ...
              <div className="comments p-2 lg:w-4/12 my-1  overflow-y-auto  rounded-md shadow-md bg-gradient-to-br from-pink-500 to-red-500  ">
                <a><button onClick={() => addtocart({ user })} className="text-center font-serif font-bold text-2xl my-1 w-full flex gap-2 justify-center bg-white p-1 rounded-lg hover:bg-green-600 hover:text-white"> Add To Cart<svg xmlns="http://www.w3.org/2000/svg" className="h-9 w-9" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                </svg></button></a>

                <h6 className="font-sans font-semibold text-center lg:mx-7 mx-2 my-5 text-gray-200 text-3xl flex gap-2 justify-center">{app.Price}<svg xmlns="http://www.w3.org/2000/svg" className="h-9 w-9" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                </svg></h6>
            ...
            <div className="bg-white h-60 lg:w-4/12  p-2 overflow-y-auto rounded-xl  bg-gradient-to-br from-yellow-300 to-red-400">
              ...
              <div className="  my-9 p-2 flex flex-row justify-between">
                <div className="flex flex-col gap-2">
                  <a > <button onClick={() => addtocart({ user })}  className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 font-serif px-2 lg:mx-6 md:mx-10 rounded flex justify-center">
                    Add To Cart <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                    </svg>
                  </button></a>
