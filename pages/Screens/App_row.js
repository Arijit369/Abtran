...
export default function App_row(props) {
  ...
  useEffect(() => {
    ...
    const subscriber = db
      .collection("Approved").where("Category","<=",props.title).where("Category",">=",props.title).limit(8)
  ...
  return (
    ...
    <div className=" md:mx-auto h-auto relative  bg-gray-100  bg-opacity-50   ">
      {/* <p className="mx-2 font-serif text-2xl font-bold  underline" onClick={() => { props.changeState(props.title) }}>{props.title}</p>
        ...
        <div className=" shop flex py-5 lg:px-2 sm:px-2 md:px-2 space-x-1 gap-1 sm:gap-4 xl:gap-8 md:gap-8 text-2xl whitespace-rowrap   overflow-x-scroll scrollbar-hide">
          {/* starts here */}
...

          {
            apps.map((i, index) => (
              ...
              <Slide right><div key={index}>

                <div className=" mx-5 my-1 md:my-2 xl:my-5 cursor-pointer   scale-125  hover:z-50    rounded-lg" >


                <a onClick={() => props.ShowSignleapp(`${i.id}`)}><Image src={i.Image1} alt={i.Appname} className="object-contain rounded-lg hover:scale-115  " height={120} width={180} layout='responsive' /></a>


                  <div className="links rounded-b-lg">
                    <div className="text-center  hover:scale-105">
                      <p className=" font-serif text-sm px-1  ">{i.Appname.slice(0,12)}..</p>
                    </div>
                    <div className="flex  sm:flex-row justify-between  ">
                      <a onClick={() => props.ShowSignleapp(`${i.id}`)}><Itemicons Icon={ViewGridIcon} /></a>
