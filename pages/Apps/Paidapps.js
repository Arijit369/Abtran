...
export default function Paidapps({ user }) {
...
const [price , setprice]=useState('')
  ...
  const handelUpload = () => {
    ...
    const uplaodTask = storage.ref(`Paidapps/${image.name}`).put(image);
    uplaodTask.on(
      "state_changed",
      snapshot => {
  ...
  const handelUpload1 = () => {
    ...
    const uplaodTask = storage.ref(`paidimages/${image1.name}`).put(image1);
    uplaodTask.on(
      "state_changed",
      snapshot => {
        const progress = Math.round(
  ...
  const handelUpload2 = () => {
    ...
    const uplaodTask = storage.ref(`paidimages/${image2.name}`).put(image2);
    uplaodTask.on(
      "state_changed",
      snapshot => {
        const progress = Math.round(
      ...
      () => {
        storage.ref("paidimages")
          ...
          .then(url2 => {
            ...
            setUrl2(url2)
            window.alert("App Image  Uploaded Successfully,Click Ok To Continue")
          })
      }
    );
  };
  ...
  const SubmitDetails = async () => {
    if (!category || !appname || !desc || !privacy || !version || !Marketing ||!Audience ||!language || !age  || !price) {
      setalert('Please Enter All Fields')

    } else {

      try {
        await db.collection('Paidapps').add({
          Category: category,
          Appname: appname,
          Description: desc,
        ...
        window.alert(`dear ${user.displayName} app submitted to Abtaran`)
    ...
    <div>
      ...
      <Fade>
        <div className=" container p-2 mb-24 ">
          <h1 className="text-gray-800 font-bold font-serif italic text-xl text-justify">Upload Paid App From Here Don't Refresh Page, You Will Lose Data</h1>
          {/* form */}
          <div className="  flex justify-center lg:mt-5   ">
            ...
            <Slide right>
              <div className=" p-3 mx-1 lg:w-3/4  pt-7 rounded-3xl shadow-xl  ">
                ...
                {/* upload logo */}
                {
                  url?<><div className=" flex flex-col sm:flex-row ">
                 ...
                 {url1 ?<><div className=" flex flex-col sm:flex-row ">
...
<div className="p-2 my-2 shadow-lg ">
  ...
  <div className='my-1 p-1 flex flex-row justify-center'>
    ...
    {/* my-1 p-1 flex flex-row justify-center */}
    <div className="px-1 border-b border-green-900 lg:w-1/2   ">
...
<div className='relative'>
  <select value={age} onChange={(e) => setage(e.target.value)} className="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
    <option>Select One</option>
    <option>Age 5+</option>
    <option>Age 15+</option>
    <option>Age 19+</option>
    <option>
      Everyone</option>
  ...
  <div className="px-1  flex w-full justify-center">
    <Slide right>
      <button onClick={SubmitDetails} className="bg-blue-500 hover:bg-blue-600 text-white font-serif font-semibold hover:text-white h-12 mt-10 py-3 border px-3 mx-2 border-blue-500 hover:border-transparent rounded flex">
        Submit   &nbsp;<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
