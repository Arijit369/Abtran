...
export default function any ({user}) {
  ...
  const loadcoments = async () => {
    const comments = await db.collection('Approved').doc(`${Blogid}`).collection('comments').orderBy('time', 'desc').get()
    setAllComments(comments.docs.map(docSnap => docSnap.data()));
    const download = await db.collection('Approved').doc(`${Blogid}`).collection('downloads').get()
    setAlldownloads(download.docs.map(docSnap => docSnap.data()))
    const like = await db.collection('Approved').doc(`${Blogid}`).collection('Likes').get()
  ...
  const loadapps = async () => {
    try {
      const docRef = db.collection("Approved").doc(Blogid);
  ...
  const Approve = async () => {
    ...
    if (!Email || !myComment) {
      ...
      await db.collection('Approved').doc(`${Blogid}`).collection('comments').add({
        text: myComment,
        email: Email,
      ...
      const commentQuery = await db.collection('Approved').doc(`${Blogid}`).collection('comments').get()
  ...
  const Download = async () => {
    await db.collection('Approved').doc(`${Blogid}`).collection('downloads').add({
      text: 1,
      time: serverTimestamp()
  ...
  const Likes = async () => {
    await db.collection('Approved').doc(`${Blogid}`).collection('Likes').add({
      text: 1,
      time: serverTimestamp()
    })
    ...
    <><div className="font-sans bg-cover bg-no-repeat w-full h-auto">
      <nav className="sticky  nv bg-opacity-90 w-full z-10  ">
        <div className="flex p-1  gap-3 text-lg  flex-row justify-between ">
          ...
          <h2 onClick={() => router.push('/')} className="border-2 border-white px-2 py-1 rounded-md cursor-pointer text-lg my-2 font-serif transition duration-100 mt-2  transform  text-gray-200 hover:font-bold active:text-red-500 flex flex-row">Home</h2>

          
          <h2 onClick={() => router.push('/Screens/Latest')} className=" border-2 border-white px-2 py-1 rounded-md cursor-pointer text-lg my-2 font-serif transition duration-100 mt-2  transform  text-gray-200 hover:font-bold active:text-red-500 flex flex-row">Latest Apps</h2>
